...
| PROMO | 把重复经验提升为进化候选 | 当同一类 memory 的 occurrence_count >= 3 时生成 PROMO；PROMO 只表示“值得审查”，不是自动变更 | .skills_memory/PROMOTION_CANDIDATES.md；PROMO-xxxx | 不自动 apply；policy_candidate 不直接进入 SKILL.md |
