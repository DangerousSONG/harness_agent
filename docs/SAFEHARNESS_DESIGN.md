...
- `/apply` is limited to approved `load_skill`, `skill.regression_case`, `skill.promotion`, `skill.creation`, `tool.update`, and `file.write` reviews. `load_skill` apply executes the reviewed skill load and updates `last_loaded_skill`; it does not create a patch preview. Regression-case apply writes reviewed eval cases and records `.reviews/apply_audit.jsonl`; skill-promotion apply is refused until `skills/<skill>/eval/cases.yaml` contains both positive and negative cases for the same `source_promo_id`. Skill-creation apply writes the reviewed `SKILL.md` and eval placeholder files and records an initial skill version when the patch preview has real changes. Tool-update apply writes reviewed files under `tools/<tool_name>/` only after approval. File-write apply writes only the reviewed target and content.
- Reviewed apply endpoints block empty patch previews for file-changing review types and return `EMPTY_PATCH_PREVIEW` with `Cannot apply: patch preview is empty.` so the UI can offer regenerate/cancel instead of Continue/Apply.
- First-time Chat tool creation is not a memory/PROMO/self-evolution operation. `POST /api/tools/create` may create new `tools/<tool>/` asset files after path guard, existing-file check, secret scan, risk classification, and explicit user confirmation; differing existing files return structured `FILE_ALREADY_EXISTS` data and require a review or another explicit conflict path.
