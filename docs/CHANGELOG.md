...
- Changed skill and tool creation requests to return confirmation-required `create_skill_review` actions for `skills/<name>/SKILL.md` and `skills/<name>/eval/cases.yaml`; clicking the action calls `/api/skills/propose` to create the pending review, and approve/apply then creates the reviewed files and records an initial skill version.
