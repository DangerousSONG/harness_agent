...
Realtime external-information requests are one-shot queries by default. Questions such as `今天英伟达财报如何？`, current stock quotes, current news, and current weather use `mode=one_shot_query`: Chat checks executable Tools first, then may use a configured provider directly for a temporary query, and otherwise refuses to fabricate realtime facts. Explicit creation language such as `帮我创建一个 web_search 工具` or `新增 finance_quote tool` uses `mode=create_persistent_tool` and proposes Tool Asset files. Creating a Tool Asset is not the same as enabling runtime execution; Chat must continue to surface `asset_exists`, `handler_available`, `provider_configured`, and `executable` status and must not claim a non-executable asset was called.

When a one-shot realtime query cannot run because no executable Tool or provider is available, the primary product action is provider configuration. Non-realtime analysis and persistent Tool Asset creation are secondary actions. Financial realtime answers must include sources when provider data is available, avoid buy/sell certainty, and state that the response is not financial advice.

Web research uses a Markdown-only source pipeline. `web_research`, `web_search`, `news_search`, and `company_research` select URLs from direct user input, configured search providers, or a no-key fallback search; each selected URL is passed through crawl4ai and normalized as `{title, url, markdown, crawl_status, extracted_at, content_length}` before any model summarization. Bailian/Qwen summarization receives Markdown only. If Bailian/Qwen credentials are unavailable, Chat returns a short Markdown excerpt fallback instead of failing. URL safety blocks `file://`, localhost, loopback/private-network addresses, secret-looking paths, and paywall/login bypass requests.

Create and Evolve are separate routes. Create is used for first-time assets such as tools, skills, docs, eval cases, workflows, and config templates. Create does not write learning memory, create PROMO candidates, or enter the self-evolution flow. Evolve is reserved for existing skills that accumulate repeated corrections through memory, promotion candidates, review, apply, and version records.
...
- `POST /api/tools/propose-create` generates a first-time tool creation plan without writing files.
