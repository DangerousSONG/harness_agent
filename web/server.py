...
def _tool_template(tool_name: str, description: str = "") -> dict[str, Any]:
    ...
    templates: dict[str, dict[str, Any]] = {
        ...
        "web_search": {
            "template": "web_search",
            "description": "Search the web, crawl selected URLs with crawl4ai, convert pages to Markdown, and return cited summaries.",
        ...
        "web_research": {
            ...
            "description": "Research URLs or web search results by crawling pages with crawl4ai, converting to Markdown, and summarizing with Bailian/Qwen when configured.",
            ...
            "outputs": {
                ...
                "urls_selected": {"type": "array"},
                "crawled_pages": {"type": "array"},
                "summary": {"type": "string"},
                "summary_provider": {"type": "string"},
                "citations": {"type": "array"},
            },
            "provider_requirements": [],
            "safety": [
                "Never pass raw HTML to the model; summarize Markdown generated by crawl4ai.",
                "Preserve title, url, markdown, crawl_status, extracted_at, and content_length for every crawled URL.",
                "Block file://, localhost, loopback, private-network, paywall-bypass, and secret-looking URLs.",
                "If Bailian/Qwen is not configured, return a Markdown excerpt fallback instead of failing.",
            ],
            "eval_cases": ["direct_url", "provider_search_then_crawl", "no_key_fallback_search", "bailian_missing_fallback", "unsafe_url_blocked"],
        },
        "finance_quote": {
...
def _supervisor_intent_route(message: str, safety: dict[str, Any]) -> dict[str, Any]:
    ...
    mode = "one_shot_query" if primary in {
        "web_search_query",
    ...
    if primary == "tool_creation_request":
        mode = "create_persistent_tool"
    elif _looks_like_realtime_capability_preference(message):
        mode = "configure_realtime_capability"
...
def _handle_chat(ctx: WebContext, message: str, context: dict[str, Any]) -> dict[str, Any]:
    ...
    if execution_intent == "external_realtime_query":
        ...
        if not registry_status.get("executable"):
            if tool_name == "weather_query" and tool_name in ctx.tool_registry.handlers:
                inputs = {"city": city, "date": "today", "units": "metric", "language": "zh-CN"}
                run_result = _run_one_shot_provider(ctx, tool_name, inputs)
                provider_trace = _trace(
                    ...
                    tool_name=tool_name,
                    method="internal",
                    path=f"provider://{tool_name}",
                    summary=_tool_run_summary(tool_name, run_result),
                )
                if run_result.get("ok"):
                    return done(_chat_result(
                        ...
                        used_skill,
                        skill_reason,
                        _weather_result_message(run_result.get("result", {})),
                        "No durable memory was captured.",
                        trace=[
                            *runtime_trace,
                            provider_trace,
                            _trace("decision", "Decision", status="completed", summary="Used the weather provider for a one-shot query without creating a Tool asset."),
                        ],
                        data={
                            "tool_name": tool_name,
                            "mode": "one_shot_query",
    ...
    if execution_intent in {"web_search_query", "financial_research_query", "finance_quote_query", "company_research_query", "news_query"}:
        return done(_handle_realtime_tool_query(
            ...
            skill_reason,
            base_trace,
            mode=str(intent_route.get("mode", "one_shot_query")),
        ))
...
def _handle_realtime_tool_query(
    ...
    registry_trace = [
        *base_trace,
        _trace(
            ...
            mode=mode,
            summary=f"Routed {mode} realtime query to tools/providers: {route_summary}.",
        ),
        *[
            _trace(
                "tool_registry_check",
    ...
    if not executable:
        ...
        provider_tools = _one_shot_provider_tools(ctx, intent, tool_names, message)
        if provider_tools:
            provider_results = []
            provider_trace = []
            for tool_name in provider_tools:
                inputs = _inputs_for_realtime_tool(tool_name, intent, message)
                result = _run_one_shot_provider(ctx, tool_name, inputs)
            ...
            if successful:
                ...
                return _chat_result(
                    ...
                    used_skill,
                    skill_reason,
                    _realtime_answer_message(intent, message, successful, []),
                    "No durable memory was captured.",
                    risk="safe_read",
                    trace=[
                        *registry_trace,
                        *provider_trace,
                        *_web_research_traces(successful),
                        _trace("decision", "Decision", status="completed", summary="Used a configured provider for a one-shot realtime query without creating a Tool asset."),
                        _trace("sources", "Sources / citations", status="completed", source_count=len(sources), summary=", ".join(sources[:4]) or "Provider result sources captured."),
            ...
            return _chat_result(
                ...
                used_skill,
                skill_reason,
                _all_realtime_tools_failed_message(intent, provider_results, statuses),
                "No durable memory was captured.",
                risk="safe_read",
                actions=_realtime_missing_tool_actions(intent, tool_names, statuses),
                trace=[
                    *registry_trace,
                    *provider_trace,
                    _trace("decision", "Decision", status="blocked", summary="Realtime answer blocked because the configured one-shot provider failed."),
                    _trace("risk_note", "Risk note", status="completed", summary=_financial_risk_note(intent)),
                ],
                data={
                    "intent": intent,
        ...
        return _chat_result(
            ...
            used_skill,
            skill_reason,
            _missing_realtime_tools_message(intent, missing_tools, statuses, mode),
            "No durable memory was captured.",
            risk="safe_read",
            actions=_realtime_missing_tool_actions(intent, tool_names, statuses),
            trace=[
                *registry_trace,
                _trace(
                    ...
                    "Decision",
                    status="blocked",
                    summary="Realtime answer blocked by missing executable tools or provider configuration.",
                ),
                _trace(
                    "risk_note",
                    "Risk note",
            ...
            data={
                ...
                "suggested_actions": [action["label"] for action in _realtime_missing_tool_actions(intent, tool_names, statuses)],
            },
        )
    ...
    if not successful:
        return _chat_result(
            ...
            used_skill,
            skill_reason,
            _all_realtime_tools_failed_message(intent, run_results, statuses),
            "No durable memory was captured.",
            risk="safe_read",
            actions=_realtime_missing_tool_actions(intent, tool_names, statuses),
            trace=[
                *registry_trace,
                *run_trace,
                _trace("decision", "Decision", status="blocked", summary="Realtime answer blocked because all available realtime providers failed."),
                _trace("risk_note", "Risk note", status="completed", summary=_financial_risk_note(intent)),
            ],
            data={"intent": intent, "mode": mode, "tool_results": run_results, "tool_statuses": statuses},
        )
    sources = _sources_from_tool_results(successful)
    return _chat_result(
        "tool_result",
...
def _tool_names_for_realtime_query(intent: str, message: str) -> list[str]:
    ...
    if intent == "financial_research_query":
        return ["finance_quote", "web_search", "news_search"]
...
def _missing_realtime_tools_message(
    intent: str,
    ...
    if intent in {"financial_research_query", "finance_quote_query"}:
        ...
        status_note = _tool_status_sentence(statuses)
        if mode == "configure_realtime_capability":
            return (
                "要长期查询美股财报，关键不是先创建 tool.yaml，而是先配置可用的搜索或金融数据 provider。"
                "我当前没有可执行的 web_search、news_search 或 finance_quote provider，不能可靠查询最新财报、股价或新闻。"
                f"当前状态：{missing_text}。{status_note}"
                "你可以先配置搜索/金融 provider；也可以让我在不使用实时数据的情况下给你一个财报分析框架。"
                "\n\n说明：这不是财务建议；在没有实时数据前，我不能编造股价、新闻、财报或分析师观点。"
            )
        return (
            "这是实时财报/市场问题，我当前没有可执行的 web_search、news_search 或 finance_quote provider，"
            "不能可靠查询今天最新财报、股价和新闻。"
            f"当前状态：{missing_text}。{status_note}"
            "你可以先配置搜索/金融 provider，或者我可以在不使用实时数据的情况下给你一个财报分析框架。"
            "\n\n"
            "说明：这不是财务建议；在没有实时数据前，我不能编造股价、新闻、财报或分析师观点。"
        )
    ...
    status_note = _tool_status_sentence(statuses)
    return (
        "这是一个需要实时外部信息的问题。我当前没有可执行的搜索/新闻 provider，不能可靠查询最新信息。"
        f"当前状态：{missing_text}。{status_note}"
        "你可以先配置搜索 provider，或者改为不使用实时数据的通用分析。"
    )


def _realtime_missing_tool_actions(intent: str, tool_names: list[str], statuses: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    statuses = statuses or []
    actions = [
        _action(
            ...
            "configure_provider",
            False,
            {"provider_type": "finance" if intent in {"financial_research_query", "finance_quote_query"} else "search"},
            kind="configure_provider",
            primary=True,
        ),
        _action("Ask without realtime data", "LOCAL", "ask_without_realtime_data", False, kind="ask_without_realtime_data"),
    ]
    for status in statuses:
        if status.get("asset_exists") and not status.get("executable"):
            name = status.get("name", "")
            actions.append(_action("Test tool", "POST", f"/api/tools/{name}/run", False, {"inputs": {}}, kind="test_tool"))
            actions.append(_action("Open tool details", "GET", f"/api/tools/{name}", False, kind="open_tool"))
            break
    if "web_search" in tool_names or intent in {"financial_research_query", "news_query", "company_research_query", "web_search_query"}:
        actions.append(_persistent_tool_action("web_search"))
    if "finance_quote" in tool_names or intent in {"financial_research_query", "finance_quote_query"}:
        actions.append(_persistent_tool_action("finance_quote"))
    return actions


def _persistent_tool_action(tool_name: str) -> dict[str, Any]:
    return _action(
        f"Create persistent {tool_name} tool",
        "POST",
        "/api/tools/create",
        True,
        {
            "tool_name": tool_name,
            "description": _default_tool_description(tool_name),
            "confirmed": True,
            "note": "Creating a Tool Asset does not automatically provide realtime access; a handler, provider configuration, and passing tests are still required.",
        },
        kind="create_tool",
        risk="medium",
        primary=False,
...
def _tool_status_sentence(statuses: list[dict[str, Any]]) -> str:
    if not statuses:
        return ""
    details = []
    for status in statuses:
        details.append(
            "{name}: asset_exists={asset_exists}, handler_available={handler_available}, "
            "provider_configured={provider_configured}, executable={executable}".format(
                name=status.get("name", ""),
    ...
    return "Tool status: " + "；".join(details) + "。"


def _one_shot_provider_tools(ctx: WebContext, intent: str, tool_names: list[str], message: str = "") -> list[str]:
    if re.search(r"https?://", message) and "web_search" in ctx.tool_registry.handlers:
        return ["web_search"]
    if intent in {"financial_research_query", "news_query", "company_research_query", "web_search_query"}:
        for candidate in ("web_search", "news_search", "company_research"):
            if candidate in tool_names and candidate in ctx.tool_registry.handlers:
                return [candidate]
    if intent == "finance_quote_query" and "finance_quote" in tool_names and _finance_provider_configured(ctx):
        return ["finance_quote"]
    return []


def _run_one_shot_provider(ctx: WebContext, tool_name: str, inputs: dict[str, Any]) -> dict[str, Any]:
    handler = ctx.tool_registry.handlers.get(tool_name)
    if not handler:
        return {
            "ok": False,
            "tool_name": tool_name,
            "error_code": "handler_missing",
            "message": f"{tool_name} provider handler is not available.",
        }
    result = handler(inputs)
    if not result.get("ok", True):
        return {"ok": False, "tool_name": tool_name, **result}
    return {"ok": True, "tool_name": tool_name, "result": result.get("result", result)}


def _search_provider_configured(ctx: WebContext) -> bool:
...
def _finance_provider_configured(ctx: WebContext) -> bool:
    env = ctx.tool_registry.env
...
def _provider_settings_summary(ctx: WebContext) -> dict[str, Any]:
    ...
    return {
        "search": {
            ...
            "provider": "mock" if str(env.get("WEB_SEARCH_MOCK_RESULTS", "") or "").strip() else str(env.get("SEARCH_PROVIDER", "") or ""),
            "runtime": "one-shot provider" if _search_provider_configured(ctx) else "not configured",
        },
        "finance": {
            "configured": _finance_provider_configured(ctx),
            "provider": str(env.get("FINANCE_PROVIDER", "") or env.get("FINANCE_QUOTE_PROVIDER", "") or ""),
            "runtime": "one-shot provider" if _finance_provider_configured(ctx) else "not configured",
        },
        "weather": {
            "configured": "weather_query" in ctx.tool_registry.handlers,
            "provider": "Open-Meteo",
            "runtime": "one-shot provider and Tool runtime handler",
        },
    }


def _looks_like_realtime_capability_preference(message: str) -> bool:
    return _looks_like_memory_request(message) and _has_any(
        message,
        ["美股", "财报", "股价", "新闻", "搜索", "联网", "provider", "finance", "stock", "earnings", "news"],
    )
...
def _tool_run_summary(tool_name: str, result: dict[str, Any]) -> str:
    if result.get("ok"):
        ...
        if tool_name == "weather_query":
            payload = result.get("result", {})
            return f"Retrieved weather for {payload.get('city', '')} from {payload.get('source', '')}."
...
def _web_research_traces(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    traces: list[dict[str, Any]] = []
    for item in results:
        payload = item.get("result", {})
        pages = payload.get("crawled_pages", [])
        if not isinstance(pages, list):
            continue
        urls = [str(page.get("url", "")) for page in pages if isinstance(page, dict) and page.get("url")]
        statuses = [
            f"{page.get('url', '')}: {page.get('crawl_status', '')}"
            for page in pages
            if isinstance(page, dict)
        ]
        markdown_count = sum(1 for page in pages if isinstance(page, dict) and page.get("markdown"))
        traces.extend([
            _trace(
                "search_mode",
                "Search mode",
                ...
                summary=f"Search mode: {payload.get('search_mode', '')}.",
            ),
            _trace(
                "urls_selected",
                "URLs selected",
                ...
                urls=", ".join(urls),
                summary=", ".join(urls[:5]) or "No URLs selected.",
            ),
            _trace(
                "crawl4ai",
                "crawl4ai crawl status",
                status="completed" if markdown_count else "failed",
                crawl_status="; ".join(statuses),
                summary=f"crawl4ai returned Markdown for {markdown_count}/{len(pages)} URL(s).",
            ),
            _trace(
                "markdown_extracted",
                "Markdown extracted",
                status="completed" if markdown_count else "failed",
                content_length=sum(int(page.get("content_length", 0) or 0) for page in pages if isinstance(page, dict)),
                summary=f"Markdown pages extracted: {markdown_count}.",
            ),
            _trace(
                "summarizer",
                "Summarized with Bailian/Qwen",
                status="completed" if payload.get("summary_provider") == "bailian_qwen" else "waiting",
                provider=payload.get("summary_provider", ""),
                summary=payload.get("summary_note", "") or "Summary generated from Markdown.",
            ),
        ])
    return traces
...
def _realtime_answer_message(intent: str, message: str, results: list[dict[str, Any]], statuses: list[dict[str, Any]]) -> str:
    ...
    markdown_summary = _markdown_summary_from_search_payloads(search_payloads)
    ...
    if intent == "financial_research_query":
        ...
        return "\n\n".join(parts)
    if source_lines:
        summary = ("\n\nMarkdown 摘要：\n" + markdown_summary) if markdown_summary else ""
        return "我查到的近期相关信息来源如下：\n" + "\n".join(source_lines) + summary + "\n\n请基于这些来源继续核验；我不会编造新闻或来源。"
...
def _markdown_summary_from_search_payloads(search_payloads: list[dict[str, Any]]) -> str:
    summaries = []
    for payload in search_payloads:
        summary = str(payload.get("summary", "") or "").strip()
        note = str(payload.get("summary_note", "") or "").strip()
        if summary:
            summaries.append(summary + (f"\n\n{note}" if note else ""))
    return "\n\n".join(summaries[:2])


def _all_realtime_tools_failed_message(intent: str, results: list[dict[str, Any]], statuses: list[dict[str, Any]] | None = None) -> str:
    errors = "; ".join(f"{item.get('tool_name')}: {item.get('error_code') or item.get('message')}" for item in results)
    status_note = _tool_status_sentence(statuses or [])
    if intent in {"financial_research_query", "finance_quote_query"}:
        return (
            f"我尝试调用实时工具，并已尝试可用的 no-key fallback search，但 provider 不可用或返回失败：{errors}。"
            f"{status_note}请配置搜索/金融 provider，或直接提供 URL 让我用 crawl4ai 抓取。"
            "我不会编造股价、新闻、财报或分析师观点。这不是财务建议。"
        )
    return (
        f"我尝试调用实时搜索工具，并已尝试可用的 no-key fallback search，但 provider 不可用或返回失败：{errors}。"
        f"{status_note}请配置搜索 provider，或直接提供 URL 让我用 crawl4ai 抓取。我不会编造来源或新闻。"
    )
...
def _tool_design_answer(message: str) -> str:
    ...
    if template.get("capability") in {"web_search", "internet_search", "browser_search"}:
        return "\n".join(
            [
                ...
                "注意：创建 Tool Asset 不会自动获得联网能力；还需要 handler、provider 配置和测试通过后，Chat 才能调用。",
            ]
        )
    if template.get("capability") == "finance_quote":
        return "\n".join(
            [
                f"我理解你想创建一个财报/行情查询工具，建议命名为 {tool_name}。",
                "它适合作为 Tool，因为它依赖外部市场数据 provider，而不是可直接写进 Skill 的静态知识。",
                "",
                "将创建：",
                f"- tools/{tool_name}/tool.yaml",
                f"- tools/{tool_name}/README.md",
                ...
                "",
                f"Provider requirements: {provider_text}",
                "Capability: query quote or earnings-related market data and return source, retrieved_at, and normalized fields.",
                "Risk level: medium, confirmation required before writing files.",
                "注意：创建 Tool Asset 不会自动获得实时金融数据能力；还需要 handler、provider 配置和测试通过。金融回答必须标注非财务建议。",
            ]
        )
    if _has_any(message, ["\u5929\u6c14", "weather"]):
        return "\n".join(
    ...
    return "\n".join(
        [
            ...
            f"Capability: {template.get('capability') or tool_name}",
            "Risk level: medium, confirmation required before writing files.",
            "注意：创建 Tool Asset 不会自动获得外部能力；还需要 handler、provider 配置和测试通过后，Chat 才能调用。",
        ]
    )
...
def _infer_tool_request(message: str) -> dict[str, Any]:
    ...
    if _has_any(message, ["web_research", "网页研究", "网页总结", "crawl4ai"]):
        candidates.append("web_research")
        reason = "Web research maps to crawl4ai-backed web_research."
    if _has_any(message, ["财报", "美股", "股价", "finance quote", "stock quote", "earnings"]):
        candidates.append("finance_quote")
        reason = "Financial quote or earnings lookup maps to finance_quote."
